  ...
  else
  {
    ...
    switch (command)
    {
      ...
      ************************************************************************************/
      ...
      case 0x0006:
      {
        status = LinxDev->UartClose(ListenerChan);
        StatusResponse(commandPacketBuffer, responsePacketBuffer, status);
        break;
      }
      case 0x0007: //Set UART Listener Interface Max Baud
        {
      ...
      ** TMC5160
      ****************************************************************************************/
      case 0x0090:// Motor Init
      
        status = LinxDev->Motor_Init(commandPacketBuffer[6]);
        StatusResponse(commandPacketBuffer, responsePacketBuffer, status);
        break;
      
      case 0x0091:// Motor Config Parameters ** Add parameters here**
        {
          para.push_back({});
          unsigned int key = commandPacketBuffer[6];
          switch (key)
          {
            case 0: 
             {
              para.at(0).Velocity= commandPacketBuffer[7] << 24|commandPacketBuffer[8]<<16|commandPacketBuffer[9]<<8|commandPacketBuffer[10];
              Velocity = para.at(0).Velocity;
              break;
             }
            case 1:
             {
              para.at(0).Accel=  commandPacketBuffer[7] << 24|commandPacketBuffer[8]<<16|commandPacketBuffer[9]<<8|commandPacketBuffer[10];
              Accel= para.at(0).Accel;
              break;
             }
            case 2:
             {
              para.at(0).Dccel=  commandPacketBuffer[7] << 24|commandPacketBuffer[8]<<16|commandPacketBuffer[9]<<8|commandPacketBuffer[10];
              Dccel= para.at(0).Dccel;
              break;
             }             
            case 3:
             {
              para.at(0).uStep=  commandPacketBuffer[7] << 24|commandPacketBuffer[8]<<16|commandPacketBuffer[9]<<8|commandPacketBuffer[10];
              uStep= para.at(0).uStep;
              break;
             } 
            case 4:
             {
              para.at(0).IRun=  commandPacketBuffer[7] << 24|commandPacketBuffer[8]<<16|commandPacketBuffer[9]<<8|commandPacketBuffer[10];
              IRun= para.at(0).IRun;
              break;
             } 
            case 5:
             {
              para.at(0).Hold_current=  commandPacketBuffer[7] << 24|commandPacketBuffer[8]<<16|commandPacketBuffer[9]<<8|commandPacketBuffer[10];
              Hold_current= para.at(0).Hold_current;
              break;
             }
            case 6:
             {
              para.at(0).pullup=  commandPacketBuffer[7] << 24|commandPacketBuffer[8]<<16|commandPacketBuffer[9]<<8|commandPacketBuffer[10];
              pullup= para.at(0).pullup;
              break;
             }  
            case 7:
             {
              para.at(0).Pos=  commandPacketBuffer[7] << 24|commandPacketBuffer[8]<<16|commandPacketBuffer[9]<<8|commandPacketBuffer[10];
              Pos= para.at(0).Pos;
              break;
             } 
      ...
      case 0x0092:// Config
        {              
          //uint8_t Motor_Id = commandPacketBuffer[6] ;
          status = LinxDev->Motor_config(commandPacketBuffer[6], Velocity, Accel,Dccel,IRun,Hold_current,uStep, pullup,Pos );
          StatusResponse(commandPacketBuffer, responsePacketBuffer, status);
          break;
        }
      case 0x0093:// Move 
        { 
          uint8_t Motor_Id = commandPacketBuffer[11] ;
          uint32_t Velocity = commandPacketBuffer[6] << 24|commandPacketBuffer[7]<<16|commandPacketBuffer[8]<<8|commandPacketBuffer[9];
          unsigned char Direction = commandPacketBuffer[10] ;
          status = LinxDev->Move(Motor_Id,Velocity, Direction);
          StatusResponse(commandPacketBuffer, responsePacketBuffer, status);
          break;
        }
      case 0x0094:// Move Stop 
        {
         // uint8_t Motor_Id = commandPacketBuffer[6] ;
          status = LinxDev->Motor_stop(commandPacketBuffer[6]);
          StatusResponse(commandPacketBuffer, responsePacketBuffer, status);
          break;
        }
      case 0x0095:// Move Absolute 
        {
          uint8_t Motor_Id = commandPacketBuffer[14] ;
          int32_t Position = commandPacketBuffer[6] << 24|commandPacketBuffer[7]<<16|commandPacketBuffer[8]<<8|commandPacketBuffer[9];
          uint32_t Velocity_ramp  = commandPacketBuffer[10] << 24|commandPacketBuffer[11]<<16|commandPacketBuffer[12]<<8|commandPacketBuffer[13];
          status = LinxDev->Move_Absolute( Motor_Id, Position, Velocity_ramp);
          StatusResponse(commandPacketBuffer, responsePacketBuffer, status);
         break;
        }
      case 0x0096:// Move Relative 
        {
          uint8_t Motor_Id = commandPacketBuffer[14] ;
          int32_t step_np = commandPacketBuffer[6] << 24|commandPacketBuffer[7]<<16|commandPacketBuffer[8]<<8|commandPacketBuffer[9];
          uint32_t Velocity_ramp  = commandPacketBuffer[10] << 24|commandPacketBuffer[11]<<16|commandPacketBuffer[12]<<8|commandPacketBuffer[13];
          int32_t *steps= &step_np;
          status = LinxDev->Move_Relative(Motor_Id, steps, Velocity_ramp);
          /*
          responsePacketBuffer[5] = (posi>> 24) & 0xFF;
          responsePacketBuffer[6] = (posi >> 16) & 0xFF;
          responsePacketBuffer[7] = (posi >> 8) & 0xFF;
          responsePacketBuffer[8] =  posi & 0xFF;
          PacketizeAndSend(commandPacketBuffer, responsePacketBuffer,4,L_OK);
          */
          StatusResponse(commandPacketBuffer, responsePacketBuffer, status);
          break;
        }
      case 0x0097: //Get data 
       {
        
       // uint8_t Motor_Id = commandPacketBuffer[6] ;
        uint32_t d_data=LinxDev->Get_Data(commandPacketBuffer[6]);
        responsePacketBuffer[5] = (d_data>> 24) & 0xFF;
        responsePacketBuffer[6] = (d_data >> 16) & 0xFF;
        responsePacketBuffer[7] = (d_data >> 8) & 0xFF;
        responsePacketBuffer[8] =  d_data & 0xFF;
        /*
        int* d_data;
        d_data=LinxDev->Get_Data( Motor_Id);
        //responsePacketBuffer[5] = (d_data[0]>> 24) & 0xFF;
        //responsePacketBuffer[6] = (d_data[0] >> 16) & 0xFF;
        //responsePacketBuffer[7] = (d_data[0] >> 8) & 0xFF;
       // responsePacketBuffer[8] =  d_data[0] & 0xFF;
        responsePacketBuffer[5] = (d_data[0]>> 24) & 0xFF;
        responsePacketBuffer[6] = (d_data[0] >> 16) & 0xFF;
        responsePacketBuffer[7] = (d_data[0] >> 8) & 0xFF;
        responsePacketBuffer[8] = d_data[0] & 0xFF;
        responsePacketBuffer[9] = (d_data[1]>> 24) & 0xFF;
        responsePacketBuffer[10] = (d_data[1] >> 16) & 0xFF;
        responsePacketBuffer[11] = (d_data[1] >> 8) & 0xFF;
        responsePacketBuffer[12] = d_data[1] & 0xFF;
        */
        PacketizeAndSend(commandPacketBuffer, responsePacketBuffer, 4, L_OK);
        break;
       }
      case 0x0098:  
       {
        //uint8_t Motor_Id = commandPacketBuffer[6] ;
        status=LinxDev->Position_clear(commandPacketBuffer[6]);
        StatusResponse(commandPacketBuffer, responsePacketBuffer, status);
        break;
      }
      case 0x0099:
      {
        uint8_t resp = LinxDev->Sensor(commandPacketBuffer[6],commandPacketBuffer[7],commandPacketBuffer[8]);
        responsePacketBuffer[5] = resp;
        PacketizeAndSend(commandPacketBuffer, responsePacketBuffer, 1, L_OK);
        break;
      }
      case 0x009A:
      {
        uint32_t Home_vel =  commandPacketBuffer[9] << 24|commandPacketBuffer[10]<<16|commandPacketBuffer[11]<<8|commandPacketBuffer[12];
        status= LinxDev->Ref_serach(commandPacketBuffer[6],commandPacketBuffer[7],commandPacketBuffer[8],Home_vel,commandPacketBuffer[13]);
        StatusResponse(commandPacketBuffer, responsePacketBuffer, status);
        break;
      }
      /****************************************************************************************
